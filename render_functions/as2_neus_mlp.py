...
@IMPLICIT_FUNCTIONS.register_module(name='as2_neus_mlp')
class AS2NeusMlp(HyperNetwork):
    """
    ...
    def __init__(
        ...
        Initialize
        """
        super(AS2NeusMlp, self).__init__()
    ...
    def compute_density(
        ...
        rays_densities, xyz_features = self.density_layer(rays_points_template)
    ...
    def forward(
        ...
        if kwargs['valid'] == False:
            ...
            #embeds_xyz = self.harmonic_embedding_xyz(x)
            sdf, xyz_features = self.density_layer(x)
            ...
            sdf, xyz_features = self.density_layer(rays_points_template)
        ...
        if enable_deformation:
            ...
            cat_features = torch.cat([deformer_features, self.intermediate_linear(xyz_features)], dim=-1)  # (n, i, p, dx + dd)
