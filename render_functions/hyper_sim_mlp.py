...
@IMPLICIT_FUNCTIONS.register_module(name='hyper_sim_mlp')
class HyperSimMlp(HyperNetwork):
    """
    ...
    def __init__(
        ...
        Initialize
        """
        super(HyperSimMlp, self).__init__()
        ...
        self.mlp_diffuse = HyperBlocks(
            dim_in=(dim_hidden_density + dim_hidden_deformer),
        ...
        intermediate_specular = torch.nn.Linear(
            dim_hidden_deformer + dim_hidden_density + dim_hidden_diffuse,
            dim_hidden_specular
        )
        _init_weight(intermediate_specular)
        self.intermediate_specular = torch.nn.Sequential(
            intermediate_specular,
            torch.nn.LayerNorm(dim_hidden_specular) if norm else torch.nn.Identity()
        )

        if n_blocks_specular > 0:
            self.mlp_specular = ResBlocks(
                dim_in=(dim_hidden_specular + embedding_dim_dir),
                dim_hidden=dim_hidden_specular,
                n_blocks=n_blocks_specular,
                norm=norm,
                act='leaky_relu'
            )
        else:
            self.mlp_specular = LinearBlock(
                dim_in=(dim_hidden_specular + embedding_dim_dir),
                dim_out=dim_hidden_specular,
                norm=norm,
                act='leaky_relu'
            )

        self.specular_layer = torch.nn.Linear(dim_hidden_specular, 3)
    ...
    def forward(
        ...
        if enable_deformation:
            ...
            rays_diffuse, diffuse_features = self._get_diffuse_colors(cat_features)  # (n, i, p, 3)
