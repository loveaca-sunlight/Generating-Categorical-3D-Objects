...
@IMPLICIT_FUNCTIONS.register_module(name='as2_neus_mlp_fix')
class AS2NeusMlpFix(HyperNetwork):
    """
    ...
    def __init__(
        ...
        Initialize
        """
        super(AS2NeusMlpFix, self).__init__()
        ...
        self.mlp_diffuse = HyperBlocks(
            dim_in=(dim_hidden_density + embedding_dim_dir + 6),
    ...
    def forward(
        ...
        if kwargs['valid'] == False:
            ...
            gradients = self._get_gradient(rays_points_world)

        # get rays colors when deformation is enabled
        if enable_deformation:
            # concatenate the features from mlp_xyz and deformer as input
            cat_features = xyz_features  # (n, i, p, dx + dd)

            # get color
            rays_colors = self._get_colors(cat_features, ray_bundle.directions, rays_points_world, gradients)  # (n, i, p, 3)
