...
from modules.transnerf import TransformModule, FullTransformModule
...
@IMPLICIT_FUNCTIONS.register_module(name='template_mlp')
class TemplateMlp(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        super(TemplateMlp, self).__init__()

        # Rigid transform module
        self.transformer = FullTransformModule() if full_transform else TransformModule(with_translation=True)
    ...
    def forward(
        ...
        if enable_deformation:
            ...
            n, i, p, _ = rays_points_template.shape
            rays_colors = rays_points_template.new_zeros((n, i, p, 3))
