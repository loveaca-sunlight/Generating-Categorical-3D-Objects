...
from modules.transform import TransformModule, FullTransformModule
...
@IMPLICIT_FUNCTIONS.register_module(name='template_mlp_v2')
class TemplateMlpV2(torch.nn.Module):
    """
    Mlp that stores the density information of a template. In this implementation, we concatenate the features from
    deformation and density branch as input to color branch
    """
    def __init__(
            self,
        ...
        super(TemplateMlpV2, self).__init__()

        # Rigid transform module
        self.transformer = FullTransformModule() if full_transform else TransformModule()
        ...
        self.intermediate_linear = torch.nn.Linear(
            feature_dim + n_hidden_neurons_xyz, feature_dim
    ...
    def forward(
        ...
        if enable_deformation:
            ...
            features = torch.cat([deformer_features, xyz_features], dim=3)  # (n, i, p, dx + dd)

            # rays colors from features of deformable field and latent code
            rays_colors = self._get_colors(features, rays_directions_world, color_latent_code)
        else:
