"""
Base mlp that don't use template and alignment
"""
...
from modules.hyper_modules import HyperNetwork, HyperLinear, HyperBlocks, HyperMLP
...
@IMPLICIT_FUNCTIONS.register_module(name='base_mlp')
class BaseMlp(HyperNetwork):
    """
    ...
    def __init__(
        ...
        self.density_layer = HyperLinear(
            dim_in=dim_hidden_density,
            dim_out=1,
            dim_middle=3,
            hyper_dim_in=shape_code_dim,
            hyper_dim_hidden=hyper_dim_hidden_diffuse,
        ...
        self.mlp_diffuse = HyperBlocks(
            dim_in=(dim_hidden_density + embedding_dim_dir),
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor) -> typing.List[torch.Tensor]:
        parameters = []
        for module in (self.mlp_density, self.density_layer):
            parameters.extend(module.produce_parameters(shape_code))
        for module in (self.mlp_diffuse, self.diffuse_layer):
            parameters.extend(module.produce_parameters(color_code))
        return parameters

    def hyper_parameters(self) -> typing.Iterator[torch.nn.Parameter]:
        for module in (self.mlp_diffuse, self.diffuse_layer, self.mlp_density, self.density_layer):
    ...
    def forward(
        ...
        xyz_features = self.mlp_density(embeds_xyz)
        ...
        cat_features = xyz_features  # torch.cat([deformer_features, xyz_features], dim=-1)  # (n, i, p, dx + dd)
