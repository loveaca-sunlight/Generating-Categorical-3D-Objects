...
from renderers import StyleNeusRendererFix
...
@MODELS.register_module(name='style_neus_fix_model')
class StyleNeusFixModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, style_data='train', **kwargs):
        super(StyleNeusFixModel, self).__init__()
        ...
        self.model = StyleNeusRendererFix(
            sequences=sequences,
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        # 没有发深度信息，后续再看
        if (batch_idx == 1) and ((self.current_epoch + 1) % 2 == 0):  # log every 5 epochs
