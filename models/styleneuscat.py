...
from renderers import StyleNeusRendererCat
...
@MODELS.register_module(name='style_neus_cat_model')
class StyleNeusCatModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, style_data='train', **kwargs):
        super(StyleNeusCatModel, self).__init__()
        ...
        self.style_input = style_img[None,]

        self.model = StyleNeusRendererCat(
            sequences=sequences,
            ...
            style_epochs=self.cfg.style_epochs,
        )
    ...
    def configure_optimizers(self):
        # Configure optimizer
        param_groups = [
            {'params': self.model.shapepro_parameters(),
             'lr': self.cfg.optimizer.shapepro_lr},
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        if batch_idx % self.cfg.optimizer.accumulate_grad_batches == 0:
            #get style_img
            style_img_path = np.random.choice(self.style_img_files, 1)[0]
            with open(style_img_path, 'rb') as f:
                #这样读就是二进制的
        ...
        out, metrics = self(
            ...
            target_image=images[0][None],
            target_fg_probability=fg_probability[0][None],
            target_mask=(fg_probability[0][None] if self._mask_image else mask_crop),
            source_image=source_images,
            sequence_name=sequence_name[0],
            enable_deformation=True,
            enable_specular=True,
            style_img=self.style_input,
        ...
        if metrics.get('color_loss') != None:
            color_loss = metrics['color_loss']
        ...
        # 没有发深度信息，后续再看
        if (batch_idx == 32) and ((self.current_epoch + 1) % 5 == 0):  # log every 5 epochs
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        if batch_idx <= 3:  # log first two samples
            ...
            writer.add_text(f'val_{batch_idx}/sequence', sequence_name[0] + "__" + style_img_path, global_step=self.current_epoch + 1)
