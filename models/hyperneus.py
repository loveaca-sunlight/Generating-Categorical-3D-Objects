...
from renderers import HyperNeusRenderer
from .registry import MODELS
from .util import mask_image, choose_views, augment_source_images


@MODELS.register_module(name='hyper_neus_model')
class HyperNeusModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, **kwargs):
        super(HyperNeusModel, self).__init__()
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        # log
        for key in ['mse_fine', 'psnr_fine', 'deformation_reg_fine', 
                    'weights_reg_fine', 'deformation_loss_fine', 'weights_norm', 'deviation']:
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        if batch_idx <= 1:  # log first two samples
            ...
            for k in ['rgb_gt']:
                img = val_nerf_out[k]
                writer.add_images(f'template_{batch_idx}/{k}', torch.clamp(img, min=0.0, max=1.0),
                                  global_step=self.current_epoch + 1, dataformats='NHWC')
