...
from renderers import StyleNeusRendererSuper
...
@MODELS.register_module(name='style_super_model')
class StyleSuperModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, style_data='train', **kwargs):
        super(StyleSuperModel, self).__init__()
        ...
        #风格图片路径
        self.style_img_train_files = find_files(style_img_path[style_data], exts=['*.png', '*.jpg'])
        ...
        self.model = StyleNeusRendererSuper(
            sequences=sequences,
            ...
            super_config=self.cfg.model.super_config,
    ...
    def configure_optimizers(self):
        ...
        param_groups = [
            ...
            {'params': self.model.super_parameters(),
             'lr': self.cfg.optimizer.super_lr},
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        #get style_img
        style_img_path = np.random.choice(self.style_img_train_files, 1)[0]
        ...
        style_idx = torch.from_numpy(np.array([self.style_img_train_files.index(style_img_path)]))
        ...
        # 没有发深度信息，后续再看
        if (batch_idx == 64) and ((self.current_epoch + 1) % 2 == 0):  # log every 5 epochs
            writer = self.logger.experiment
            for k in ['rgb_fine','rgb_gt','rgb_ori']:
                img = out[k]
