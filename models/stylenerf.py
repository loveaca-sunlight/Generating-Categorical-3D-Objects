...
from renderers import StyleNerfRenderer
...
@MODELS.register_module(name='style_nerf_model')
class StyleNerfModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, style_data='train', **kwargs):
        super(StyleNerfModel, self).__init__()
        ...
        if self.cfg.model.get('model.ray_sampler_config',None) == None:
    ...
    def load_shape(self, path):
        shape_weights = torch.load(path, map_location='cpu')['state_dict']
        self.model.load_shape(shape_weights)
