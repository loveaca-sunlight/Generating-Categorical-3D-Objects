...
from renderers import HyperRenderer
from .registry import MODELS
from .util import mask_image, choose_views, augment_source_images


@MODELS.register_module(name='hyper_nerf_model')
class HyperNeRFModel(pl.LightningModule):
    def __init__(self, cfg, sequences: typing.List[str], load_pretrained_weights=True, **kwargs):
        super(HyperNeRFModel, self).__init__()
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        for key in ['mse_coarse', 'mse_fine', 'psnr_coarse', 'psnr_fine', 'deformation_reg_fine', 'weights_reg_coarse',
                    'weights_reg_fine', 'deformation_loss_fine', 'weights_norm']:
