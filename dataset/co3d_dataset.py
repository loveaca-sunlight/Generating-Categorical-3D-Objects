...
class Co3dDataset(torch.utils.data.Dataset):
    ...
    Args:
        ...
        load_key_points: Enable loading key points
    """
    ...
    def __post_init__(self):
        ...
        # load key points
        if self.load_key_points:
            self._load_key_points_data(os.path.dirname(self.frame_annotations_file))
        self._filter_db()  # also computes sequence indices
        print(str(self))

    def _load_key_points_data(self, root_dir: str):
        """
        ...
        self._key_points = {
            seq_name: os.path.join(kp_dir, f'{seq_name}.pt')
            for seq_name in self.seq_annots.keys()
            if os.path.exists(os.path.join(kp_dir, f'{seq_name}.pt'))
        }
    ...
    def __getitem__(self, index):
        ...
        # add key point data to meta
        if self.load_key_points:
            frame_data.meta['key_points'] = torch.load(
                self._key_points[frame_data.sequence_name],
                map_location='cpu'
            )
    ...
    def _filter_db(self):
        ...
        # filter out sequences without key points
        if self.load_key_points:
            print('Removing sequences without key points...')
            old_seq_len = len(self.seq_annots)
            self.seq_annots = {
                name: entry
                for name, entry in self.seq_annots.items()
                if entry.sequence_name in self._key_points.keys()
            }
            print("... filtered %d -> %d" % (old_seq_len, len(self.seq_annots)))
