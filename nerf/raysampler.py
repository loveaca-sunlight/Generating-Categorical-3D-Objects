...
import torch
from pytorch3d.renderer import RayBundle  # MonteCarloRaysampler, NDCGridRaysampler
from pytorch3d.renderer.cameras import CamerasBase

from modules.ray_sampler import NerfMonteCarloRaysampler, NerfNDCGridRaysampler
...
class NeuSRaysampler(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        Args:
            ...
            negative_z: true - camera rays are along negative z direction
        """
        ...
        self._grid_raysampler = NerfNDCGridRaysampler(
        ...
        self._mc_raysampler = NerfMonteCarloRaysampler(
            min_x=-1.0,
            max_x=1.0,
            min_y=-1.0,
            max_y=1.0,
    ...
    def _stratify_ray_bundle(self, ray_bundle: RayBundle):#替换深度 z_vals
        """
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.valida = kwargs['valid']

        if (camera_hash is None) and (not caching) and self.valida:
            # Sample random rays from scratch.
            ray_bundle = self._mc_raysampler(cameras,
                                             fg_probability=kwargs.pop('fg_probability'),
                                             fg_thr=kwargs.pop('fg_thr'),
            ...
            if camera_hash is not None:
                ...
                # We generate a full ray grid from scratch. Shape (n, h, w, d)
                full_ray_bundle = self._grid_raysampler(cameras, bounds=kwargs.pop('bounds', None))
                full_ray_bundle = self._normalize_raybundle(full_ray_bundle)

            n_pixels = full_ray_bundle.directions.shape[1: -1].numel()  # (h * w)
            ...
            if self.valida:
                ...
                # chunksize means (batch_size * rays_per_image)
                if chunksize is None:
                    # if chunksize is not given, take all the rays
                    chunksize = n_pixels * batch_size

                assert chunksize % batch_size == 0  # evenly distribute rays to images
                chunksize = chunksize // batch_size  # rays per image
            ...
            assert sel_rays.numel() > 0, 'No ray is selected.'

            # Take the "sel_rays" rays from the full ray bundle.
            ray_bundle = RayBundle(
                *[
                    v.contiguous().view(batch_size, n_pixels, -1)[:, sel_rays, :]  # v.view(n_pixels, -1)[sel_rays]
        ...
        if (
            (self._stratified and self.valida)
            or (self._stratified_test and not self.valida)
        ) and not caching:  # Make sure not to stratify when caching!
            ray_bundle = self._stratify_ray_bundle(ray_bundle)

        return ray_bundle

class BatchRaysampler(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #if (camera_hash is None) and (not caching) and self.valida:
        if (camera_hash is None) and (not caching) and (self.valida or kwargs.get('ray_sampler_config',None) != None):
            # Sample batch rays from scratch.
            ori_ray_bundle = self._grid_raysampler(cameras,
                                             fg_probability=kwargs.pop('fg_probability'),
                                             fg_thr=kwargs.pop('fg_thr'),
                                             bounds=kwargs.pop('bounds', None),
                                             ray_sampler_config=kwargs.get('ray_sampler_config',None)
                                             )
            full_ray_bundle = self._normalize_raybundle(ori_ray_bundle)
            n_pixels = full_ray_bundle.directions.shape[1: -1].numel()
            ray_bundle = RayBundle(
                *[
                    v.contiguous().view(batch_size, n_pixels, -1)  # v.view(n_pixels, -1)[all_rays]
                    # .view(batch_size, sel_rays.numel() // batch_size, -1)
                    .to(device)
                    for v in full_ray_bundle
                ]
            ...
            if camera_hash is not None:
                ...
                # We generate a full ray grid from scratch. Shape (n, h, w, d)
                ori_ray_bundle = self._grid_raysampler(cameras, bounds=kwargs.pop('bounds', None))
                full_ray_bundle = self._normalize_raybundle(ori_ray_bundle)
            ...
            ori_ray_bundle = RayBundle(
                *[
                    v.contiguous().view(batch_size, n_pixels, -1)[:, sel_rays, :]  # v.view(n_pixels, -1)[sel_rays]
                    # .view(batch_size, sel_rays.numel() // batch_size, -1)
                    .to(device)
                    for v in ori_ray_bundle
                ]
            )
        ...
        return ray_bundle,ori_ray_bundle
