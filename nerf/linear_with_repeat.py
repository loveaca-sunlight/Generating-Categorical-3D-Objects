...
class LinearWithRepeat(torch.nn.Linear):
    ...
    def forward(self, input: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        output2 = F.linear(input[1], self.weight[:, n1: n1+n2], None)
        if len(input) == 3:
          output3 = F.linear(input[2], self.weight[:, n1+n2:], None)
          return output1 + output2.unsqueeze(-2) + output3
        elif len(input) > 3:
          output3 = F.linear(input[2], self.weight[:, n1+n2:n1+n2+3], None)
          output4 = F.linear(input[3], self.weight[:, n1+n2+3:], None)
          return output1 + output2.unsqueeze(-2) + output3 + output4
        else:
          return output1 + output2.unsqueeze(-2) #+ output3
