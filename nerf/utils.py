...
def calc_mse(x: torch.Tensor, y: torch.Tensor, m: torch.Tensor = None):
    """
    Calculates the mean square error between tensors `x` and `y`. m indicates the valid area (1) and invalid area (0)
    """
    error = (x - y) ** 2
    if m is not None:
        m = m.expand_as(error)
        return (m * error).sum() / m.sum()
    else:
        return torch.mean(error)


def calc_psnr(x: torch.Tensor, y: torch.Tensor, m: torch.Tensor = None):
    """
