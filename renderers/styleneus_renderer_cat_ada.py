...
class StyleNeusRendererCat(RendererBase):
    ...
    def rest_parameters(self):
        ...
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in encoder2_parameters) and (param not in hyper_color_parameters) and (param not in hyper_shape_parameters) and (param not in deviation_parameters):
                yield param
    ...
    def encode_codes(self, source_images: torch.Tensor):
        shape_code, color_code_mean, color_code = self.encoder(source_images)

        return shape_code, color_code_mean, color_code
    ...
    def forward(
        ...
        if enable_deformation:
            shape_latent_code, color_latent_code, color_code = self.encode_codes(source_image)
        else:
            assert not self.training
        ...
        # get style code
        with torch.no_grad():
            _, style_latent_code, style_code = self.encode_codes(style_image)

        style_feat = self.style_encoder.adaptive_instance_normalization(content_feat = color_code, style_feat = style_code)
        # generate weights
        hyper_parameters = []
        if produce_parameters and shape_latent_code is not None:
            hyper_parameters = self.produce_parameters(shape_latent_code.clone().detach(), style_feat.clone().detach())
        else:
            assert not self.training
