...
class PrimRenderer(RendererBase):
    """
    ...
    def __init__(
        ...
        self.deformer = PrimDeformableField(
            **deformer_config
        )
    ...
    def hyper_parameters(self):
        ...
        for func in self._implicit_function.values():
            yield from func.mlp_diffuse.hyper_net.parameters()
        yield from self.deformer.mlp.hyper_net.parameters()
    ...
    def generate_weights(self, shape_code: torch.Tensor, color_code: torch.Tensor):
        self.deformer.generate_weights(shape_code)
        for func in self._implicit_function.values():
            func.generate_weights(color_code)
    ...
    def _process_ray_chunk(
        ...
        for renderer_pass in ("coarse", "fine"):
            ...
            # store
            if self.training:
                points_specular[renderer_pass] = self._implicit_function[renderer_pass].latest_points_specular
    ...
    def forward(
        ...
        Args:
            ...
            generate_new_weights: whether to generate new weights
        """
        ...
        if generate_new_weights:
            if shape_latent_code is not None:
                prim_weights.append(self.deformer.generate_weights(shape_latent_code))
            if color_latent_code is not None:
                for render_pass in ('coarse', 'fine'):
                    prim_weights.append(self._implicit_function[render_pass].generate_weights(color_latent_code))
        ...
        if self.training:
            ...
            for render_pass in ('coarse', 'fine'):
                metrics[f'specular_reg_{render_pass}'] = calc_l2_reg(points_specular[render_pass])

            # Calc l1 norm of generated weights
            hyper_norm = 0.0
            for group in prim_weights:
                for w in group:
                    hyper_norm += weight_norm_l2(w)
            metrics['weights_norm'] = hyper_norm
