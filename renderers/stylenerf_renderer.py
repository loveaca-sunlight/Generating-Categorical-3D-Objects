...
class StyleNerfRenderer(RendererBase):
    """
    ...
    def load_shape(self, state_dict: dict):
        with torch.no_grad():
            for module_name, module in [('mlp_xyz', self.volumetric_function.mlp_xyz),
                                        ('density_layer', self.volumetric_function.density_layer),]:
                for name, param in module.named_parameters():
    ...
    def _renderer(
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                sdf = self.volumetric_function.get_sdf_withpts(pts, enable_deformation=False)
                sdf = sdf.squeeze(-1)
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, xys, new_z_vals, sdf, last=False):
        ...
        if not last:
            new_sdf = self.volumetric_function.get_sdf_withpts(new_pts, enable_deformation=False)
    ...
    def forward(
        ...
        if produce_parameters and shape_latent_code is not None:
            ...
            self.volumetric_function.produce_parameters(style_code)
        else:
            assert not self.training
