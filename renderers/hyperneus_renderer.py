...
class HyperNeusRenderer(RendererBase):
    ...
    def rest_parameters(self):
        ...
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in hyper_parameters) and (param not in deviation_parameters):
                yield param
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor):
        ...
        #parameters.extend(self.volumetric_function.produce_parameters(torch.cat((color_code,shape_code),dim=1)))
        return parameters
    ...
    def forward(
        ...
        if not self.valida:
            ...
            out = {
                ...
                for k in ("rgb_fine", "rgb_gt", 'normal')
            }
            rot = np.linalg.inv(target_camera.R[..., :3, :3].detach().cpu().numpy())
            #normal_img = (np.matmul(rot[1][None, :, :], out['normal'][0].reshape(-1,3)[:, :, None].cpu().numpy())
            #                 .reshape([128, 128, 3, -1]) * 128 + 128).clip(0, 255)
            #cv.imwrite(os.path.join(),normal_img[..., 0])
            for i in range(batch_size):
                out['normal'][i] = torch.from_numpy( np.matmul(rot[i][None, :, :], out['normal'][i].reshape(-1,3)[:, :, None].cpu().numpy()).reshape([128, 128, 3, -1]) )[...,-1]
