...
class StyleNeusRendererCat(RendererBase):
    """
    ...
    def __init__(
        ...
        self.style_epochs = style_epochs
        #init_val = 550.
    ...
    def _process_ray_chunk(
        ...
        (features, weights, gradient, gradient_error), ray_bundle_out, ori_ray_bundle = self._renderer(
        ...
        if not self.training:
            if transform_code == None:
                coor = depth * ori_ray_bundle.directions + (ori_ray_bundle.origins / 8.)
            else:
                coor = (depth / transform_code[0][-1]) * ori_ray_bundle.directions + (ori_ray_bundle.origins / 8.)
        ...
        if not self.training:
            out = {"rgb_fine": rgb_fine, "rgb_gt": rgb_gt, 'normal': normal, 'coor': coor}
        else:
    ...
    def _renderer(
        ...
        # the rendering rays.
        ray_bundle, ori_ray_bundle = self.raysampler(
        ...
        if self.n_importance > 0:
            with torch.no_grad():
                sdf = self.volumetric_function.get_sdf_withpts(pts, enable_deformation=kwargs['enable_deformation'])
                sdf = sdf.squeeze(-1)
                for i in range(self.up_sample_steps):
                    ...
                    z_vals, sdf = self.cat_z_vals(rays_o=rays_o,
                                                  ...
                                                  enable_deformation=kwargs['enable_deformation'])
        ...
        return (color, weights, gradients.reshape(batch_size, chunk, n_samples, 3), gradient_error), ray_bundle, ori_ray_bundle
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, xys, new_z_vals, sdf, last=False, enable_deformation=True):
        batch_size, chunk, n_samples = z_vals.shape
        ...
        if not last:
            new_sdf = self.volumetric_function.get_sdf_withpts(new_pts, enable_deformation=enable_deformation)
    ...
    def forward(
        ...
        if enable_deformation:
            ...
            assert not self.training
            _, color_latent_code = self.encode_codes(source_image)
        ...
        if produce_parameters and shape_latent_code is not None:
            ...
            assert not self.training
            hybrid_code = self.shape_pro(torch.cat((color_latent_code.clone().detach(),style_code),dim=1))
            self.volumetric_function.produce_parameters(hybrid_code)#color
        ...
        if self.valida:
            ...
            content_loss, style_loss = self.style_encoder.cal_style_loss(content_out,style_out,style_image,self._n_rays_per_image,self._n_rays_per_image)

            metrics['content_loss'] = content_loss
            if epoch < self.style_epochs:
            ...
            out = {
                k: torch.cat(
                    [ch_o[k] for ch_o in chunk_outputs],
                    dim=1,
                ).view(-1, self._n_rays_per_image, self._n_rays_per_image , 3)
                if chunk_outputs[0][k] is not None
                else None
