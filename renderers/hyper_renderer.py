...
class HyperRenderer(RendererBase):
    """
            ...
            for render_pass in ['coarse', 'fine']:
                for module_name, module in [('mlp_xyz', self._implicit_function[render_pass].mlp_xyz),
                    ...
                    for name, param in module.named_parameters():
                        weight_name = f'_implicit_function.fine.{module_name}.{name}'
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor):
        ...
        for func in self._implicit_function.values():
            parameters.extend(func.produce_parameters(color_code))
        return parameters
    ...
    def compute_point_density(
        ...
        # only use fine model
        fine_model = self._implicit_function['fine']
