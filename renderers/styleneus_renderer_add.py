...
class StyleNeusRendererAdd(RendererBase):
    """
    ...
    def __init__(
        ...
        encoder_config['dim_in'] = 3

        # Encoder2
        self.style_for = ENCODERS.build(encoder_config)
        print(f'Latent Code Encoder: {type(self.style_for).__name__}')
    ...
    def encoder2_parameters(self):
        ...
        :return:
        """
        return self.style_for.parameters()
    
    def encoder3_parameters(self):
        """
        Return parameters of encoder
        :return:
        """
    ...
    def rest_parameters(self):
        ...
        encoder3_parameters = set(self.encoder3_parameters())
        ...
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in encoder2_parameters) and (param not in encoder3_parameters) and \
    ...
    def forward(
        ...
        # get style code
        #with torch.no_grad():
        _, style_code = self.style_for(style_image)

        rate = 0.5
        if epoch < 200:
            weight = rate * (epoch/200)
        else:
            weight = rate
        style_code_add = color_latent_code * (1-weight) + style_code * weight

        # generate weights
        hyper_parameters = []
        if produce_parameters and shape_latent_code is not None:
            hyper_parameters = self.produce_parameters(shape_latent_code.clone().detach(), style_code_add)
        else:
            assert not self.training
        ...
        if self.valida:
            ...
            color_loss = F.mse_loss(color_latent_code, style_code)

            metrics['color_loss'] = color_loss
