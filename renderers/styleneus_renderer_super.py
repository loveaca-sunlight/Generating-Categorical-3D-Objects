...
class StyleNeusRendererSuper(RendererBase):
    """
    ...
    def __init__(
        ...
        # 超分模块
        self.SuperResolution = SuperResolution(**super_config)
    ...
    def super_parameters(self):
        ...
        """
        #for func in self.volumetric_function:
        yield from self.SuperResolution.parameters()
    ...
    def rest_parameters(self):
        ...
        super_parameters = set(self.super_parameters())
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in encoder2_parameters) and \
                        (param not in hyper_color_parameters) and (param not in hyper_shape_parameters) \
                            and (param not in deviation_parameters) and (param not in super_parameters):
                yield param
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor):
        ...
        #parameters.extend(self.volumetric_function.produce_parameters(torch.cat((color_code,shape_code),dim=1)))#color
    ...
    def _process_ray_chunk(
        ...
        new_xy_grid = torch.stack(
                        tuple(
                            reversed(
                                torch.meshgrid(
                                    torch.linspace(1.0, -1.0, self._n_rays_per_image *2, dtype=torch.float32),
                                    torch.linspace(-1.0, 1.0, self._n_rays_per_image *2, dtype=torch.float32),
                                )
                            )
                        ),
                        dim=-1,
                    ).view(-1,(self._n_rays_per_image *2)**2,2).cuda()
        ...
        #feature 0-35 rays_color  36 depth 37-39gradient
        rgb = features[..., : 3]
        feature = features[..., : 32]
        depth = features[..., 32: 33]
        #normal = features[..., 4: 7]
        rgb_S = self.SuperResolution(rgb, feature)
    ...
    def forward(
        ...
        if produce_parameters and shape_latent_code is not None:
            hyper_parameters = self.produce_parameters(shape_latent_code.clone().detach(), style_code)
        else:
            assert not self.training
        ...
        for k in ['rgb_fine','rgb_gt','depth_fine','rgb_ori']:
            img = out[k]
            if len(img.shape) < 4:
                img_size = img.shape[1]
                img_wh = int(img_size**0.5)
                out[k] = img.view(batch_size, img_wh, img_wh, -1).permute(0,3,1,2)
        ...
        if self.valida:
            ...
            content_loss, style_loss = self.style_encoder.cal_style_loss(content_out,style_out,style_image,self._n_rays_per_image * 2,self._n_rays_per_image * 2,trans=False)
            ...
            if epoch < 1:#先只用mesloss让他训练一下超分能力
                metrics['content_loss'] = 0.
            else:
            ...
            '''out = {
                k: torch.cat(
                    ...
                    dim=1,
                ).view(-1, self._n_rays_per_image * 2, self._n_rays_per_image * 2 , 3)
