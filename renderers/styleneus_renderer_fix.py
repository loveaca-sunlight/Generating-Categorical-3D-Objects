...
class StyleNeusRendererFix(RendererBase):
        ...
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in encoder2_parameters) and \
                          (param not in hyper_color_parameters) and (param not in deviation_parameters):
                yield param
    ...
    def produce_parameters(self, style_code: torch.Tensor):
        parameters = []
    ...
    def _renderer(
        ...
        '''if kwargs['transform_code'] is not None:
            ...
            if kwargs['enable_trans']:
                ray_bundle = self.volumetric_function.transformer(ray_bundle, kwargs['transform_code'])
                ray_bundle = self.volumetric_function.transformer(ray_bundle, self._get_transform_code("427_60021_116224"))
            else:
                ray_bundle = self.volumetric_function.transformer(ray_bundle, kwargs['transform_code'])'''
        if kwargs['transform_code'] is not None:
            #ray_bundle = ray_bundle
    ...
    def cat_z_vals(self, rays_o, rays_d, z_vals, xys, new_z_vals, sdf, last=False):
        ...
        if not last:
            new_sdf = self.volumetric_function.get_sdf_withpts(new_pts)
    ...
    def forward(
        ...
        '''if enable_trans:
            ...
            rigid_latent_code = self._get_transform_code("427_60021_116224") #无变化'''
        if enable_trans:
            rigid_latent_code = (self._get_transform_code(sequence_name) if transform_code is None else transform_code)
        else:
        ...
        # generate weights
        hyper_parameters = []
        hyper_parameters = self.produce_parameters(style_code)
