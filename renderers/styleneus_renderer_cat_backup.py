...
class StyleNeusRendererCat(RendererBase):
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor, style_code: torch.Tensor):
        ...
        parameters.extend(self.volumetric_function.produce_parameters(torch.cat((color_code,style_code),dim=1)))#color
    ...
    def forward(
        ...
        if produce_parameters and shape_latent_code is not None:
            hyper_parameters = self.produce_parameters(shape_latent_code.clone().detach(), color_latent_code.clone().detach(), style_code.clone().detach())
        else:
            assert not self.training
