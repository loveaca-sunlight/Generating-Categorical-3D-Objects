...
class StyleCreaRenderer(RendererBase):
    """
    ...
    def forward(
            ...
            style_image1: torch.Tensor = None,
            style_image2: torch.Tensor = None,
        ...
        with torch.no_grad():
            style_code1 = dropout(self.style_encoder(style_image1))
            style_code2 = self.style_encoder(style_image2)
        
        for i in range(512):
            if style_code1[0][i] == 0:
                style_code1[0][i] = style_code2[0][i]


        # generate weights
        hyper_parameters = []
        if produce_parameters and shape_latent_code is not None:
            hyper_parameters = self.produce_parameters(shape_latent_code.clone().detach(), style_code1.clone().detach())
        else:
            assert not self.training
        ...
        if self.valida:
            ...
            content_loss, style_loss = self.style_encoder.cal_style_loss(content_out,style_out,style_image,self._n_rays_per_image)
