...
from .util import select_template_weights_hyper
...
class StyleNeusRenderer(RendererBase):
    """
    ...
    def __init__(
        ...
        # get style code
        with torch.no_grad():
            style_code = self.style_encoder(style_img)
        self.style_color_code = torch.nn.parameter.Parameter(style_code,requires_grad=True)
    ...
    def template_parameters(self):
        ...
        :return:
        """
        if 'Res' in self.volumetric_function.__class__.__name__:
    ...
    def rest_parameters(self):
        ...
        for param in self.parameters():
            if (param not in template_parameters) and (param not in transform_parameters) and \
                    (param not in encoder_parameters) and (param not in encoder2_parameters) and (param not in hyper_parameters) and (param not in deviation_parameters):
                yield param
    ...
    def produce_parameters(self, shape_code: torch.Tensor, color_code: torch.Tensor):
        ...
        parameters.extend(self.volumetric_function.produce_parameters(color_code))#color
        return parameters
    ...
    def forward(
        ...
        hyper_parameters = []
        if produce_parameters and self.style_color_code is not None:
            hyper_parameters = self.produce_parameters(self.style_shape_code, self.style_color_code)
        else:
            assert not self.training
        ...
        if not self.valida:
            ...
            content_out = {
                k: torch.cat(
                    [ch_o[k] for ch_o in content_chunk_outputs],
                    dim=1,
                ).view(-1, *self._image_size, 3)
                if content_chunk_outputs[0][k] is not None
                else None
            ...
            style_out = {
                k: torch.cat(
                    [ch_o[k] for ch_o in style_chunk_outputs],
                    dim=1,
                ).view(-1, *self._image_size, 3)
                if style_chunk_outputs[0][k] is not None
                else None
                for k in ("rgb_fine", "rgb_gt")
            }
            ...
            content_out = content_chunk_outputs[0]
            style_out = style_chunk_outputs[0]
        ...
        if self.valida:
            # mse loss
            # None
            content_input = torch.cat((content_out['rgb_fine'], content_out['depth_fine']), dim=2)
            style_input = torch.cat((style_out['rgb_fine'], style_out['depth_fine']), dim=2)

            # Style and Content loss
            content_loss, style_loss = self.encoder2.cal_style_loss(content_input,style_input,style_image,self._n_rays_per_image)
            ...
            points_deformation = style_out['points_deformation']
            points_position = style_out['points_position']
        ...
        return style_out, content_out, metrics
